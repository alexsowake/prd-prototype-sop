...
| 依赖 | 用途 | 必须？ | 缺失时动作 |
|---|---|---|---|
| `/enhance-prompt` skill | 优化 Stitch prompt，**硬性要求** | ✅ 必须 | 提示用户：在 Claude Code 里执行 `/find-skills` 搜索 `enhance-prompt` 并安装；或联系提供此 skill 的作者 |
...
- `/enhance-prompt`：写作规范主要来源 + 术语库，**必须安装**。
- `/design-md`：从 Stitch project 提取 DESIGN.md，做跨 project 一致性用（可选）。
- `/taste-design`：DESIGN.md + 反"AI 味"禁单（可选）。
- `/stitch-loop`：多页一致性 baton 模式（可选）。
